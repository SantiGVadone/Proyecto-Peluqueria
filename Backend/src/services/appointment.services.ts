...
export const getAppointmentByRangeServices = async (
  fromDate: Date,
  toDate: Date,
  business_id: number,
) => {
  const result = await getAppointmentByRange(fromDate, toDate, business_id)
...
export const createAppointmentServices = async (
...
export const deleteAppointmentServices = async (
  id: number,
  business_id: number,
) => {
